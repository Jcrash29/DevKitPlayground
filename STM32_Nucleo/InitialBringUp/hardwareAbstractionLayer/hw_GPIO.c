...
/**
 * Take in a GPIO_ID + Port and Pin and set it as an input.
 * The GPIO pin can than be read using the GPIO_ID.
 *
 * @Note:This is setting pins into Pull Up mode. If a different setting is
 * required please use the advanced command
 */
void Gpio_initInput(hw_gpio_id_t gpio_id, GPIO_TypeDef* port, uint16_t pin )
{
  /* This simply sets most of the defaults for an output */
  Gpio_initAdvanced(gpio_id, port, pin,
		  GPIO_MODE_IT_FALLING, GPIO_NOPULL, GPIO_SPEED_FAST, 0);
